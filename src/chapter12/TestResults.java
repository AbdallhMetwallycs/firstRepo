...
public class TestResults {
    public static void main(String[] args) {
        Map<String, Integer> makeGrades = getMakeUpGrades();
        Map<String, Integer> originalGrades = getOriginalGrades();

        for(var student: makeGrades.entrySet()){
            Integer firstGrade = originalGrades.get(student.getKey());
            Integer secondGrade = makeGrades.get(student.getKey());

            if(secondGrade>firstGrade) originalGrades.put(student.getKey(), student.getValue());
        }
        System.out.println("Final Grades :");
        originalGrades.forEach((k,v)->System.out.println(k+" you get "+v+" grades"));
